/**
 * Created by js_zh on 2018/9/8.
 * 执行版本 2.0.0
 * 注：在使用最新版本不行的话，降版本到：执行版本 2.0.
 * 基于 redux做的组件，可自行修改
 */
...
import { transTime } from './time';

class player_video extends PureComponent {
    state = {
        musicUrl: ''
    };
    ...
    setDuration = (obj) => {
        console.log(obj);
        this.props.dispatch(setPlaySong({duration: obj.duration, playTime: transTime(obj.duration), playing: true}));
    };
    setTime = (time) => {
        console.log(time);
        const { duration } = this.props.currentPlay;
        this.props.dispatch(setPlaySong({currentTime: transTime(time.currentTime), sliderProgress: time.currentTime / duration}));
    };
    onEnd = () => {};
    videoError = () => {};
    onBuffer = () => {};
